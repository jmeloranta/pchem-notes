...
\end{column}
\begin{column}{3.5cm}
\href{http://en.wikipedia.org/wiki/Max_Planck}{\uline{Max Planck}} (1858 - 1947), German physicist (Nobel price 1918)
\end{column}
\end{columns}
...
\vline\hspace*{0.25cm}
\begin{column}{4cm}
\operson{einstein}{0.1}{Albert Einstein, German physicist (1879 - 1955), Nobel price 1921}
...
\begin{columns}
\begin{column}{3cm}
\operson{debroglie}{0.1}{Louis de Broglie, French physicist (1892 - 1987), Nobel price 1929}
\end{column}
\vline\hspace*{0.25cm}
...
\otext
\textbf{Excercise.} If you would consider yourself as a particle moving at 4.5 mi/h (2 m/s), what would be your de Broglie wavelength? Use classical mechanics to predict your momentum (i.e. $p = mv$). Would it make sense to use quantum mechanics in this case?
...
\aeqn{9.5}{E = \underbrace{\frac{1}{2}mv^2}_\textnormal{Kinetic energy} + \underbrace{V}_\textnormal{Potential energy} = \frac{p^2}{2m} + V = T + V}
...
\aeqn{X.3}{\psi(x) = \frac{1}{\sqrt{2\pi}}\int\limits_{-\infty}^{\infty}\psi(k)e^{-ikx}dk\textnormal{ (forward transformation)}}

\beqn{X.4}{\psi(k) = \frac{1}{\sqrt{2\pi}}\int\limits_{-\infty}^{\infty}\psi(x)e^{ikx}dx\textnormal{ with }p_x = \hbar k\textnormal{ (inverse transformation) }}{\textnormal{or }\psi(p_x) = \frac{1}{\sqrt{2\pi}}\int\limits_{-\infty}^{\infty}\psi(x)e^{ip_xx/\hbar}dx\textnormal{ and }\hbar \equiv \frac{h}{2\pi}}
