...
$$\left<\hat{p}_x\right> = \int\limits_{-\infty}^{\infty} \underbrace{\psi_0(x)}_\textnormal{even} \underbrace{\left(-i\hbar\frac{\partial}{\partial x}\right) \underbrace{\psi_0(x)}_\textnormal{even}}_\textnormal{odd} dx = 0$$
...
{\psi = \frac{N_{v_x}}{\sqrt{3}}H_{v_x}\left(\sqrt{\alpha_x}x\right)e^{-\alpha_xx^2/2}}
{ \times \frac{N_{v_y}}{\sqrt{3}}H_{v_y}\left(\sqrt{\alpha_y}y\right)e^{-\alpha_yy^2/2} \times \frac{N_{v_z}}{\sqrt{3}}H_{v_z}\left(\sqrt{\alpha_z}z\right)e^{-\alpha_zz^2/2}}
