...
\textbf{Solution.} Methane belongs to $T_d$ point group (character table given earlier, $h = 24$). The basis is four-dimensional $(H_a, H_b, H_c, H_d)$. We need to use Eq. (\ref{eq5.24}). First we obtain the $g$-factors from $T_d$ character table: $g(E) = 1, g(C_3) = 8, g(C_2) = 3, g(S_4) = 6, g(\sigma_d) = 6$. To get $\chi^{(l)}(c)$'s we use the following method:
