...
\begin{column}{7cm}
\otitle{7.5 Pauli exclusion principle}

To see where the two spin functions used in the previous section came from, we consider two electrons as an example.
The spin functions can be written as products:

\vspace*{-0.3cm}

\aeqn{7.17}{\alpha(1)\alpha(2)\textnormal{, }\beta(1)\alpha(2)\textnormal{, }\alpha(1)\beta(2)\textnormal{ or  }\beta(1)\beta(2)}
...
\aeqn{7.18}{\alpha(1)\alpha(2)\textnormal{ (symmetric if 1 and 2 are exchanged)}}

\aeqn{7.19}{\beta(1)\beta(2)\textnormal{ (symmetric)}}

\aeqn{7.20}{\frac{1}{\sqrt{2}}\left(\alpha(1)\beta(2) + \alpha(2)\beta(1)\right)\textnormal{ (symmetric)}}

\aeqn{7.21}{\frac{1}{\sqrt{2}}\left(\alpha(1)\beta(2) - \alpha(2)\beta(1)\right)\textnormal{ (antisymmetric)}}
...
\aeqn{7.22}{\psi = 1s(1)1s(2)\times\frac{1}{\sqrt{2}}\left(\alpha(1)\beta(2) - \alpha(2)\beta(1)\right)}

A generalization of this result for many-electron systems was proposed by Slater in 1929:

\aeqn{7.23}{\hspace*{-0.4cm}
...
\otext
\textbf{Example.} Show that the Slater determinant in Eq. (\ref{eq7.23}) for the ground state helium atom is an eigenfunction of the total spin operator $\hat{S}_{z,tot} = \hat{S}_{z_1} + \hat{S}_{z_2}$, where 1 and 2 refer to the two electrons.

\otext
\textbf{Solution.} First we recall how $\hat{S}_z$ operates on electron spin as follows:

$$\hat{S}_z|\alpha\rangle = +\frac{\hbar}{2}|\alpha\rangle\textnormal{ and }\hat{S}_z|\beta\rangle = -\frac{\hbar}{2}|\beta\rangle$$

Next, we expand the Slater determinant in Eq. (\ref{eq7.23}):
...
\aeqn{7.25}{\psi_{sym} = \frac{1}{\sqrt{2}}\left(1s(1)2s(2) + 1s(2)2s(1)\right)\textnormal{ (symmetric)}}

\aeqn{7.26}{\psi_{asym} = \frac{1}{\sqrt{2}}\left(1s(1)2s(2) - 1s(2)2s(1)\right)\textnormal{ (antisymmetric)}}
...
\aeqn{7.27}{\psi_1 = \frac{1}{\sqrt{2}}\left(1s(1)2s(2) - 2s(1)1s(2)\right)\alpha(1)\alpha(2)}

\aeqn{7.28}{\psi_2 = \frac{1}{2}\left(1s(1)2s(2) - 2s(1)1s(2)\right)\left(\alpha(1)\beta(2) + \beta(1)\alpha(2)\right)}

\aeqn{7.29}{\psi_3 = \frac{1}{\sqrt{2}}\left(1s(1)2s(2) - 2s(1)1s(2)\right)\beta(1)\beta(2)}
...
\aeqn{7.30}{\psi_4 = \frac{1}{2}\left(1s(1)2s(2) + 2s(1)1s(2)\right)\left(\alpha(1)\beta(2) - \alpha(2)\beta(1)\right)}
...
\otext
For helium atom, the Pauli principle relevant for the excited states but, for example, for lithium the principle is important for the ground state. The Slater determinant for Li can be written as (expansion of the determinant looks already quite complicated):

\aeqn{7.32}{\psi = \frac{1}{\sqrt{6}}
\begin{vmatrix}
1s(1)\alpha(1) & 1s(1)\beta(1) & 2s(1)\alpha(1)\\
1s(2)\alpha(2) & 1s(2)\beta(2) & 2s(2)\alpha(2)\\
