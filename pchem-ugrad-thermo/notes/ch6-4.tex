...
\begin{enumerate}
\item Consider a solution with mole fraction $x_{A,1}$ of A.
\item Heat the solution to its boiling point $T_2$. The liquid mole fraction is now $x_{A,2}$.
\item Extract vapor, which has mole fraction $y_{A,2}$. Note that there is more of the volatile component in the gas phase than in the liquid.
\end{enumerate}

Repeat the above cycle for the liquid, which was condensed from the gas phase above. The next step is shown as $x_{A,3}$ and $y_{A,3}$. By repeating the cycle, a better separation of the components can be reached.
