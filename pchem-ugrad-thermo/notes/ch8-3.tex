...
where $h$ is the Planck's constant (6.626076 $\times$ 10$^{-34}$ J s), $m$ is the atom mass, $n_x$, $n_y$ and $n_z$ are quantum numbers, and $a,b,c$ define the volume where the atoms are confined (``the box''). When the quantitized levels are close to each other, we can replace the above summations by integrals, for example:
