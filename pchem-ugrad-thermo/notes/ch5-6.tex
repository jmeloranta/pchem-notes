...
\otext
From these quantities $y_i$ can be calculated: $y_i(\xi) = \frac{n_i(\xi)}{n_{tot}(\xi)} = \frac{n_{i,0} + v_i\xi}{n_{tot,0} + v\xi}$
...
\otext
2. Next consider addition of an inert gas with temperature and pressure kept constant (note that $V$ changes in this process). If $n_{inert}$ changes, the prefactor in Eq. (\ref{eq5.54}) changes as well. Since $K = K(T)$ and temperature is constant, the product term in Eq. (\ref{eq5.54}) (i.e., $K_y$) must counter balance the change. This means that the mole fractions $y_i$ must take new values (i.e., the composition of the gas mixture changes).
